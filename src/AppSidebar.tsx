    ...
    <aside className={`sidebar ${collapsed ? 'is-collapsed' : ''}`} aria-label="Main navigation">
      <SidebarHeader className="dashboard-sidebar-header">
        <div className="brand-row">
          <Button variant="ghost" className="app-button brand" onClick={() => collapsed ? toggleSidebar() : navigate('Overview')} aria-label={collapsed ? 'Expand sidebar' : 'ArcRank overview'} title={collapsed ? 'Expand sidebar' : undefined}><img className="brand-wordmark sidebar-label" src="/assets/branding/arcrank-wordmark-dark.svg" alt="ArcRank" width="149" height="30" /><img className="brand-symbol" src="/assets/branding/arcrank-mark-dark.svg" alt="" width="33" height="32" />{collapsed && <ChevronRight className="brand-expand-icon" size={20} aria-hidden="true" />}</Button>
      ...
      <SidebarFooter className="sidebar-bottom">
        ...
        <Button variant="ghost" className="app-button workspace-switch" onClick={() => navigate('Settings')} title="Lavalab workspace" aria-label="Lavalab workspace settings"><Avatar className="workspace-avatar"><AvatarFallback>LL</AvatarFallback></Avatar><span className="workspace-name sidebar-label"><strong>Lavalab</strong><span>usclavalab.com</span></span><img className="workspace-chevrons sidebar-label" src="/assets/workspace-chevrons.svg" alt="" width="20" height="20" /></Button>
      </SidebarFooter>
    </aside>
  </Sidebar>
}
