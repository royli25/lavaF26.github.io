...
type QueryChartData = { id: string; label: string; summary: number | null; period: string; values: (number | null)[]; change: number | null; competitor?: { name: string; logo: string; values: number[] } }
export default function AppearanceChart({ querySearch, queryPills, queryData }: { querySearch?: ReactNode; queryPills?: ReactNode; queryData?: QueryChartData } = {}){
 ...
 const competitor=!noData?queryData?.competitor:undefined
 const competitorValue=competitor?.values[Math.round((point??1)*(competitor.values.length-1))]
 const competitorLine=competitor?.values.map((v,i)=>`${i?'L':'M'}${40+i/(competitor.values.length-1)*680},${144-v/100*122}`).join(' ')
 ...
 {point!==null&&<g><line x1={x} x2={x} y1="22" y2="144" stroke="#9a9ef0" strokeDasharray="3 4" strokeOpacity=".5"/><circle cx={x} cy={value.y} r="3.5" fill="#d4d6ff" stroke="#6571ef" strokeWidth="2"/>{competitor&&<circle cx={x} cy={144-competitorValue!/100*122} r="3.5" fill="#ffe0c7" stroke="#e0a16d" strokeWidth="2"/>}</g>}
 </svg><div className="zebra-months">{labels.map(m=><span key={m}>{m}</span>)}</div>
 {noData?<span className="zebra-empty">No results yet</span>:<span className="zebra-latest" aria-label={`${date}: ${format(value.value)} appearances${competitor ? `, ${competitor.name} ${format(competitorValue!)}` : ''}`} style={point===null&&!queryData?undefined:{left:`clamp(70px, ${x/738*100}%, calc(100% - ${point===null?38:70}px))`,right:'auto',top:`calc(${value.y/174*100}% - 10px)`,transform:'translate(-50%, -100%)'}}>{point===null?(queryData?format(value.value):'2,846'):`${date} · ${format(value.value)}`}</span>}
 </div></Panel>
}
